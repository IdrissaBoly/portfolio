...
function Home({ dark, updateDark }) {
  return (
    <Container
      className={`p-5 fw-bolder
        d-flex flex-column align-items-start min-vh-100 
        ${dark ? "bg-dark bg-opacity-75 text-light  " : ""} `}
      fluid
    >
      <Container className="p-2">
        <Row lg={2} xs={1} className="align-items-center">
          <Col>
            <h3 className={` fst-italic  ${dark ? "ColorFLuo" : ""}  `}>
              <Typed strings={["> Hey my name is"]} typeSpeed={40}></Typed>
            </h3>
            <br />
            <h2 className="fs-1">Idrissa Belko Boly</h2>
            <h2 className="fs-1">I build things for the WEB</h2>
            <p>
        ...
        <div className="row text-center col-lg-2">
          <a
            ...
            className={`${
              dark ? "email-link" : "email-link bg-dark bg-opacity-75"
            } rounded `}
          >
            Get in Touch
          </a>
